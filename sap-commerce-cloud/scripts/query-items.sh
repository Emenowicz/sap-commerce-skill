...
# Create netrc file for credential protection (avoids exposing passwords in process list)
HAC_HOST=$(echo "$HAC_URL" | sed -E 's|https?://([^:/]+).*|\1|')
NETRC_FILE=$(mktemp /tmp/hac_netrc.XXXXXX)
...
# Replace placeholder with actual query using Groovy single-quoted string (no interpolation)
ESCAPED_QUERY=$(printf '%s' "$QUERY" | sed 's/\\/\\\\/g' | sed "s/'/\\\\'/g")
...
CSRF_RESPONSE=$(curl -s -c "$COOKIE_FILE" -b "$COOKIE_FILE" \
    ...
    "${CURL_SSL_ARGS[@]}" "$HAC_URL/console/scripting" 2>/dev/null)
...
RESPONSE=$(curl -s -X POST \
    ...
    --netrc-file "$NETRC_FILE" \
    "${CURL_SSL_ARGS[@]}" "$HAC_URL/console/scripting/execute" \
